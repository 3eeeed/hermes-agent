...
describe('MarkdownImage media routing', () => {
  ...
  it('renders a context descriptor on a regular file attachment', async () => {
    const { container } = render(<MarkdownTextContent isRunning={false} text="[report](#media:%2Ftmp%2Ffactory%20review.pdf)" />)

    const contextTarget = await waitFor(() => {
      const el = container.querySelector('[data-hermes-context-file]')
